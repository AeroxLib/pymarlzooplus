# -*- coding: utf-8 -*-
# @Time    : 2024/1/17 10:55
# @Author  : Zhuohui Zhang
# @File    : tgcnet_controller.py
# @Software: PyCharm
...
class TGCNetMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        agent_outputs, weights = self.forward(ep_batch, t_ep, test_mode=test_mode)
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        return (agent_outs.view(ep_batch.batch_size, self.n_agents, -1),
                weights.view(ep_batch.batch_size, self.n_agents, -1))

    def init_hidden(self, batch_size):
        if self.args.agent == 'ic3net':
            self.hidden_states = self.agent.init_hidden().unsqueeze(0).expand(2, batch_size, self.n_agents, -1)
        else:
    ...
    def load_models(self, path):
        self.agent.load_state_dict(torch.load("{}/agent.pt".format(path), map_location=lambda storage, loc: storage))
